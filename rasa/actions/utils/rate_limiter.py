            ...
            if not self.redis or not await self.redis.ping():
                ...
                try:
                    ...
                    if self.redis:
                        ...
                        await self.redis.wait_closed() # Ensure it's closed
                    self.redis = await aioredis.from_url(self.redis_url, socket_timeout=5) # Add timeout
                    await self.redis.ping() # Verify connection
                    logger.info("Successfully connected to Redis.")
                except (aioredis.RedisError, asyncio.TimeoutError, OSError) as e:
