...
from app.models import Event, EventsSlot, Lecture, Step, Answer, User, Course, ChatSession, FlaggedResponse, Team, ChatMessage
...
def get_from_cache(key):
    ...
    with cache_lock:
        if key in cache:
            ...
            if time_module.time() - timestamp < CACHE_TIMEOUT:
                return data
            else:
                del cache[key]
    return None
...
def set_cache(key, data):
    """Set item in cache"""
    with cache_lock:
        cache[key] = (data, time_module.time())
        # Clean old entries periodically
        if len(cache) > 100:  # Simple cleanup
            current_time = time_module.time()
            to_delete = [k for k, (_, t) in cache.items() if current_time - t > CACHE_TIMEOUT]
            for k in to_delete:
                del cache[k]
...
def get_chat_history(user_id=None, limit=100):
    """
    Parse chat history from Rasa event table for a specific user.
    
    Args:
        user_id (str): The sender_id to filter events for
        limit (int): Maximum number of events to retrieve
    
    Returns:
        dict: Parsed chat history with messages organized chronologically
    """
    try:
        # Build query
        query = Event.query.order_by(Event.timestamp.asc())
        ...
        for event in events:
            ...
            if event.data:
                try:
                    event_data["data"] = json.loads(event.data)
                except:
                    event_data["data"] = {"raw": event.data}
            ...
            if event.type_name == "user":
                ...
                message = {
                    "role": "user",
                    "content": event_data["data"].get("text", ""),
                    "timestamp": event.timestamp,
                    "intent": event.intent_name
                }
                chat_history["messages"].append(message)
                chat_history["user_events"].append(event_data)
                ...
                chat_history["messages"].append(message)
                chat_history["bot_events"].append(event_data)
...
def get_chat_history_summary(user_id=None, days=7):
    """
    Get a summary of chat activity for a user over the last N days.
    ...
    try:
        ...
        # Build query
        query = Event.query.filter(Event.timestamp >= cutoff_time)
        ...
        for event in events:
            ...
            if event.type_name == "user":
                ...
                if event.intent_name:
                    summary["unique_intents"].add(event.intent_name)
            elif event.type_name == "bot":
                summary["bot_responses"] += 1
                if event.action_name:
                    summary["unique_actions"].add(event.action_name)
        
        if timestamps:
            summary["first_interaction"] = min(timestamps)
            summary["last_interaction"] = max(timestamps)
        
        # Convert sets to lists for JSON serialization
        summary["unique_intents"] = list(summary["unique_intents"])
